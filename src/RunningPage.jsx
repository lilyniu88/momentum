...
// Import MapView - Metro will use MapView.web.js for web, react-native-maps for native
...
const useLocation = (isWorkoutPaused) => {
  ...
  useEffect(() => {
    const getInitialLocation = async () => {
      try {
        // On web, set a default location and skip permission request
        if (Platform.OS === 'web') {
          setLocation({
            latitude: 39.9526,
            longitude: -75.1652,
  ...
  useEffect(() => {
    if (isLoading) return
    // Skip location tracking on web (not fully supported)
    ...
    return () => {
      if (locationSubscriptionRef.current) {
        try {
          ...
          console.warn('Error cleaning up location subscription:', e)
        }
        locationSubscriptionRef.current = null
      }
    }
        ...
        {isLoading ? (
          ...
          <View style={styles.mapPlaceholder}>
            <Text style={styles.mapPlaceholderText}>
              {Platform.OS === 'web' ? 'Map view not available on web' : 'Map unavailable'}
            </Text>
            {errorMsg && (
